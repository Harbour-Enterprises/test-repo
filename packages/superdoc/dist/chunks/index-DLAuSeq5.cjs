...
const xmlJs = require("./xml-js-D1kix40p.cjs");
const url = require("./url-B1HBJk5O.cjs");
...
var streamBrowser = xmlJs.eventsExports.EventEmitter;
var buffer_list;
...
function requireBuffer_list() {
  ...
  var _require = xmlJs.require$$2, Buffer = _require.Buffer;
  var _require2 = xmlJs.util, inspect = _require2.inspect;
...
function require_stream_writable() {
  ...
  var internalUtil = {
    deprecate: xmlJs.browser
  };
  var Stream = streamBrowser;
  var Buffer = xmlJs.require$$2.Buffer;
  ...
  xmlJs.inherits_browserExports(Writable, Stream);
  function nop() {
  }
  ...
  function onwriteError(stream2, state2, sync, er, cb) {
    ...
    if (sync) {
      vue.process$1.nextTick(cb, er);
      vue.process$1.nextTick(finishMaybe, stream2, state2);
  ...
  function onwrite(stream2, er) {
    ...
    if (er) onwriteError(stream2, state2, sync, er, cb);
      ...
      if (sync) {
        vue.process$1.nextTick(afterWrite, stream2, state2, finished, cb);
      } else {
  ...
  function prefinish2(stream2, state2) {
    if (!state2.prefinished && !state2.finalCalled) {
      if (typeof stream2._final === "function" && !state2.destroyed) {
        ...
        vue.process$1.nextTick(callFinal, stream2, state2);
      } else {
        state2.prefinished = true;
        stream2.emit("prefinish");
      }
...
function require_stream_duplex() {
  ...
  xmlJs.inherits_browserExports(Duplex2, Readable);
  {
...
function require_stream_readable() {
  ...
  xmlJs.eventsExports.EventEmitter;
  ...
  var from;
  xmlJs.inherits_browserExports(Readable, Stream);
  var errorOrDestroy2 = destroyImpl.errorOrDestroy;
  ...
  function ReadableState(options, stream2, isDuplex) {
    ...
    this.encoding = null;
    if (options.encoding) {
      if (!StringDecoder) StringDecoder = xmlJs.requireString_decoder().StringDecoder;
  ...
  function emitReadable(stream2) {
    ...
    if (!state2.emittedReadable) {
      ...
      state2.emittedReadable = true;
      vue.process$1.nextTick(emitReadable_, stream2);
    }
  }
  ...
  function maybeReadMore(stream2, state2) {
    if (!state2.readingMore) {
      ...
      vue.process$1.nextTick(maybeReadMore_, stream2, state2);
    }
  }
  ...
  function resume(stream2, state2) {
    if (!state2.resumeScheduled) {
      ...
      vue.process$1.nextTick(resume_, stream2, state2);
    }
  }
  ...
  function endReadable(stream2) {
    ...
    if (!state2.endEmitted) {
      state2.ended = true;
      vue.process$1.nextTick(endReadableNT, state2, stream2);
    }
  }
...
xmlJs.inherits_browserExports(Transform$1, Duplex);
...
xmlJs.inherits_browserExports(PassThrough, Transform);
...
var inherits$1 = xmlJs.inherits_browserExports;
  ...
  if (mode === "fetch") {
    let read = function() {
      reader.read().then(function(result) {
        ...
        self2.push(jszip.Buffer.from(result.value));
        read();
      }).catch(function(err) {
        resetTimers(true);
        if (!self2._destroyed)
      ...
      var writable = new WritableStream({
        write: function(chunk) {
          ...
          return new Promise(function(resolve, reject) {
            if (self2._destroyed) {
              reject();
            } else if (self2.push(jszip.Buffer.from(chunk))) {
              resolve();
            } else {
...
IncomingMessage$1.prototype._onXHRProgress = function(resetTimers) {
  ...
  switch (self2._mode) {
    ...
    case "arraybuffer":
      ...
      self2.push(jszip.Buffer.from(new Uint8Array(response2)));
      break;
    ...
    case "ms-stream":
      ...
      reader.onprogress = function() {
        if (reader.result.byteLength > self2._pos) {
          self2.push(jszip.Buffer.from(new Uint8Array(reader.result.slice(self2._pos))));
...
var inherits = xmlJs.inherits_browserExports;
...
var ClientRequest = request.exports = function(opts) {
  ...
  self2._headers = {};
  if (opts.auth)
    self2.setHeader("Authorization", "Basic " + jszip.Buffer.from(opts.auth).toString("base64"));
  ...
  if (self2._mode === "fetch") {
    ...
    if (capability.abortController) {
      ...
      if ("requestTimeout" in opts && opts.requestTimeout !== 0) {
        self2._fetchTimer = jszip.commonjsGlobal.setTimeout(function() {
    ...
    jszip.commonjsGlobal.fetch(self2._opts.url, {
    ...
    var xhr = self2._xhr = new jszip.commonjsGlobal.XMLHttpRequest();
    try {
...
ClientRequest.prototype._resetTimers = function(done2) {
  var self2 = this;
  jszip.commonjsGlobal.clearTimeout(self2._socketTimer);
  self2._socketTimer = null;
  if (done2) {
    jszip.commonjsGlobal.clearTimeout(self2._fetchTimer);
    self2._fetchTimer = null;
  } else if (self2._socketTimeout) {
    self2._socketTimer = jszip.commonjsGlobal.setTimeout(function() {
      self2.emit("timeout");
...
const index = /* @__PURE__ */ jszip.getDefaultExportFromCjs(streamHttp);
