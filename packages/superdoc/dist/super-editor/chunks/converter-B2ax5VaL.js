...
var _DocxExporter_instances, generate_xml_as_list_fn, replaceSpecialCharacters_fn, generateXml_fn, _SuperConverter_instances, exportNumberingFile_fn, prepareCommentsXmlFilesForExport_fn, exportProcessHeadersFooters_fn, exportProcessNewRelationships_fn, exportProcessMediaFiles_fn;
import "vue";
...
class NodeContext {
  constructor(type2, attrs, marks, solid, match, options2) {
    this.type = type2;
    this.attrs = attrs;
    this.marks = marks;
    this.solid = solid;
    ...
    this.match = match || (options2 & OPT_OPEN_LEFT ? null : type2.contentMatch);
  }
  findWrapping(node2) {
    if (!this.match) {
      if (!this.type)
    ...
    fragment.forEach((node2) => {
      if (active.length || node2.marks.length) {
        ...
        while (rendered < node2.marks.length) {
          ...
          let markDOM = this.serializeMark(add, node2.isInline, options2);
          if (markDOM) {
            active.push([add, top]);
  ...
  */
  serializeNode(node2, options2 = {}) {
    ...
    for (let i = node2.marks.length - 1; i >= 0; i--) {
      let wrap2 = this.serializeMark(node2.marks[i], node2.isInline, options2);
      if (wrap2) {
...
function canSplit(doc2, pos, depth = 1, typesAfter) {
  ...
  for (let d = $pos.depth - 1, i = depth - 2; d > base; d--, i--) {
    let node2 = $pos.node(d), index2 = $pos.index(d);
...
function objectIncludes(obj1, obj2, options2 = { strict: true }) {
  const keys2 = Object.keys(obj2);
  if (!keys2.length) return true;
  return keys2.every((key) => {
    if (options2.strict) return obj2[key] === obj1[key];
...
function translateTextNode(params) {
  ...
  const isTrackedNode = node2.marks?.some((m) => trackedMarks.includes(m.type));
  if (isTrackedNode) return translateTrackedNode(params);
  const isLinkNode = node2.marks?.some((m) => m.type === "link");
...
function translateTrackedNode(params) {
  ...
  node2.marks = marks.filter((m) => !trackingMarks.includes(m.type));
  if (trackStyleMark) {
...
function processOutputMarks(marks = []) {
  return marks.flatMap((mark) => {
    if (mark.type === "textStyle") {
      return Object.entries(mark.attrs).filter(([key, value]) => value).map(([key, value]) => {
...
function translateFieldAnnotation(params) {
  const { node: node2, isFinalDoc } = params;
  const { attrs = {} } = node2;
...
function translateContentBlock(params) {
  ...
  const drawing = {
    name: "w:drawing",
    elements: [
      ...drawingContent ? [...drawingContent.elements || []] : []
    ]
  };
  const choice = {
    name: "mc:Choice",
...
generateXml_fn = function(node2, debug = false) {
  ...
  if (elements) {
    if (name === "w:instrText") {
      ...
      const text = __privateMethod(this, _DocxExporter_instances, replaceSpecialCharacters_fn).call(this, elements[0].text);
      tags.push(text);
    } else {
      if (elements) {
...
function applyMarksToHtmlAnnotation(state2, marks) {
  ...
  doc2.descendants((node2, pos) => {
    ...
    const foundTextStyle = node2.marks.find((m) => m.type.name === "textStyle");
    const foundHighlight = node2.marks.find((m) => m.type.name === "highlight");
    if (!foundTextStyle) {
      tr.addMark(pos, pos + node2.nodeSize, textStyleType.create({
        ...fontFamily?.attrs,
        ...fontSize?.attrs
      }));
    ...
    if (!foundHighlight) {
      tr.addMark(pos, pos + node2.nodeSize, highlightType.create({
        ...highlight?.attrs
      }));
    }
  });
...
function handleTableCellNode(node2, row, table, rowBorders, columnWidth = null, styleTag, params, cellIndex) {
  const { docx, nodeListHandler: nodeListHandler2 } = params;
  ...
  if (vMergeAttrs && vMergeAttrs["w:val"] === "restart") {
    ...
    let cellIndex2 = cellsInRow.findIndex((el) => el === node2);
    let rowspan = 1;
    for (let remainingRow of remainingRows) {
      const firstCell = remainingRow.elements.findIndex((el) => el.name === "w:tc");
      const cellAtIndex = remainingRow.elements[firstCell + cellIndex2];
      ...
      rowspan++;
      remainingRow.elements.splice(firstCell + cellIndex2, 1);
    }
...
function getReferencedTableStyles(tblStyleTag, docx, nodeListHandler2) {
  ...
  styleTag.elements.find((el) => el.name === "w:uiPriority");
  let baseTblPr;
...
function handleTableRowNode(node2, table, rowBorders, styleTag, params) {
  ...
  rowHeightTag?.attributes["w:hRule"];
  const borders = {};
  ...
  const content = cellNodes?.map((n, index) => {
    ...
    return handleTableCellNode(n, node2, table, borders, colWidth, styleTag, params, index);
  }) || [];
  const newNode = {
    type: "tableRow",
    content,
...
const handlePictNode = (params) => {
  ...
  node2.elements?.find((el) => el.name === "v:shapetype");
...
function handleShapTextboxImport({
  pict,
  pNode,
  shape,
  params
...
function importCommentData({ docx }) {
  ...
  const extractedComments = allComments.map((el) => {
    ...
    const { elements: elements2 } = el;
    const commentData = elements2[0];
...
const _SuperConverter = class _SuperConverter {
  ...
  async exportToDocx(jsonData, editorSchema, documentMedia, isFinalDoc = false, commentsExportType, comments = [], editor, exportJsonOnly = false) {
...
exportNumberingFile_fn = function(params) {
...
export {
  ...
  Buffer2 as E,
  Fragment as F,
  inputRulesPlugin as G,
  TrackDeleteMarkName as H,
  TrackInsertMarkName as I,
  v4 as J,
  TrackFormatMarkName as K,
  comments_module_events as L,
  Mapping as M,
  NodeSelection as N,
  AddMarkStep as O,
  PluginKey as P,
  ...
  SuperConverter as X,
  EditorState as Y,
  hasSomeParentWithClass as Z,
  parseSizeUnit as _,
  Plugin as a,
  InputRule as a0,
  toKebabCase as a1,
  kebabCase as a2,
  docxNumberigHelpers as a3,
  parseIndentElement as a4,
  combineIndents as a5,
  getColStyleDeclaration as a6,
  SelectionRange as a7,
  Transform as a8,
  createColGroup as a9,
  commonjsGlobal as aa,
  getDefaultExportFromCjs$1 as ab,
  ...
  liftTarget as l,
  findWrapping as m,
  generateDocxRandomId as n,
  objectIncludes as o,
  minMax as p,
  chainableEditorState as q,
  replaceStep as r,
  generateRandom32BitHex as s,
  getStyleTagFromStyleId as t,
  getAbstractDefinition as u,
  getDefinitionForLevel as v,
  isMacOS as w,
  isIOS as x,
  DOMSerializer as y,
  Mark as z
